        ...
        if (current == $currentBox.index()) {
            ...
            */
            $('#littleBoxes > div').not($currentBox).each(function () {
                var $ele = $(this);
                $ele.stop().animate({
                    'top': (Math.floor(Math.random() * 862) - 100) + 'px',
                    'left': (Math.floor(Math.random() * 1512) - 450) + 'px',
                    'opacity': 0
                }, 800, function () {
                    $(this).hide();
                });
            ...
            $currentBox.find('img').attr('src', 'assets/img/' + ch + '.jpg');  //find the img in clicked currentBox and change it's src
